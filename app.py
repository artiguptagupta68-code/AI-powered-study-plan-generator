...
def parse_pdf_hierarchy(files):
    ...
    for f in files:
        lines = read_pdf(f)
        exam_name = "CUSTOM"
        # detect exam from lines
        for l in lines:
            for ex in AVAILABLE_SYLLABUS:
                if ex.upper() in l.upper():
                    exam_name = ex
                    break
            if exam_name != "CUSTOM": break
...
# -------------------------------
custom_plan = st.checkbox("Create custom syllabus (upload PDFs)", key="custom_plan")
...
if custom_plan:
    ...
    syllabus_all = parse_pdf_hierarchy([os.path.join(syllabus_root,f) for f in os.listdir(syllabus_root) if f.endswith(".pdf")])
    if exam_name not in syllabus_all:
        st.error(f"No syllabus found for {exam_name}.")
        st.stop()
    syllabus_json = syllabus_all[exam_name]
