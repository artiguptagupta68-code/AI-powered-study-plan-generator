...
def is_subtopic(line):
    return len(line.split()) > 12 or any(x in line for x in ["-", ":", "(", "1.", "2.", "i.", "ii."])

# ---------------------------------
# UNIFIED PARSER (DEFAULT + UPLOAD)
# ---------------------------------
...
if mode == "Available syllabus":
    exam = st.selectbox("Select Exam", ["Junior Engineer", "GATE", "NEET"])
...
# ---------------------------------
    ...
    uploads = st.file_uploader("Upload syllabus PDFs", type=["pdf"], accept_multiple_files=True)
...
for d_idx, day in enumerate(st.session_state.calendar):
    ...
    if st.button("Mark Day Completed", key=f"done_{d_idx}"):
        if carry and d_idx + 1 < len(st.session_state.calendar):
            st.session_state.calendar[d_idx + 1]["plan"] = carry + st.session_state.calendar[d_idx + 1]["plan"]
