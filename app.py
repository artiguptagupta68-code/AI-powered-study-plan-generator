...
def read_pdf(file):
    """Read PDF from path or uploaded file and return clean lines"""
...
def parse_uploaded_syllabus(files):
    """Convert uploaded PDFs into JSON: subject -> topic"""
    data = defaultdict(list)
    for f in files:
        lines = read_pdf(f)
        subject = None
        for l in lines:
            # Detect subject lines (all uppercase words)
            if l.isupper() and l.replace(" ","").isalpha():
                subject = l.title()
            elif subject:
                # Split topics by comma or semicolon
                parts = [p.strip() for p in re.split(r"[;,]", l) if len(p.strip())>3]
...
custom_plan = st.checkbox("Create a custom syllabus plan", key="custom_plan")
...
if custom_plan:
    ...
    st.info("Default syllabus is not included in this demo. Upload your syllabus PDFs instead.")
    exam_name = st.text_input("Enter Exam Name", key="default_exam")
    uploaded_files = st.file_uploader(
        f"Upload syllabus PDF(s) for {exam_name}",
        type=["pdf"],
        accept_multiple_files=True
...
def build_queue():
    ...
    for s in selected_subjects:
        for t in syllabus_json[s]:
            q.append({"subject":s,"topic":t,"subtopic":t,"time_min":estimate_time_min(t, exam_name)})
    return q
